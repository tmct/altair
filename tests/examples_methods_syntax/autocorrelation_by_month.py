...
(
    alt.Chart(source, title="Autocorrelation of daily max temperature")
    .transform_window(
        window=[
    ...
    .transform_calculate(month="month(datum.date) + 1")
    .transform_fold(
    ...
    .encode(
        x=alt.X("month:O", title="Month", axis=alt.Axis(labelExpr=month_labels)),
        y=alt.Y(
            "corr:Q",
        ...
        color=alt.Color(
            ...
            legend=alt.Legend(labelExpr=lag_labels),
        ),
    )
)
